...
class GitHubIssueWorkItem(WorkItem):
    ...
    async def resume(self, ctx: WorkItemContext) -> None:
        ...
        source = await ctx.db.get_github_source(source_id) if source_id else None
        if source:
    ...
    async def _auto_start(self, ctx: WorkItemContext, source: Any) -> None:
        ...
        token_record = await ctx.db.get_github_token(source.token_id) if source.token_id else None
        comments: list[dict[str, Any]] = []
        if token_record:
            try:
...
class GitHubIssuesSource(TaskSource):
    ...
    async def poll(self, ctx: dict[str, Any]) -> list[WorkItemDraft]:
        ...
        sources = await db.list_github_sources()
        ...
        for repo_source in sources:
            ...
            token_record = await db.get_github_token(repo_source.token_id)
            if not token_record:
                continue
            try:
